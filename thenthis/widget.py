        ...
        if not self.widget:
            ...
            for (key, value) in self.dictionary.items():
                ...
                if isinstance(value, BaseModel):
                    ...
                    key_element = widgets.Label(value=f"unknown element type ({type(value)}")
                try:
                ...
                for (name, type_, default) in self.name_type_default:
                    ...
                    if type_==str:
                        ...
                        if False:
                            ...
                            ui_element = widgets.Text(
                                value=t_to_str(value) if value else None,
                                placeholder=str(default) if default else "",
                                description=name,
                                style={"description_width": "initial"},
                            )
...
#                         ui_element.observe(self.on_datetime_change(name), names="value")
                        ...
                        ui_element = widgets.Select(
                            options = [x.value for x in ListOpMode.__dict__["_member_map_"].values()],
                            value = value_str,
                            description='list op mode',
                            disabled=False
                        )
                        ui_element.observe(self.on_list_op_mode_change(name), names="value")
                    elif type_==dict:
                        ...
                        ui_element = widgets.Label(value=f"unknown element type ({type(value)} value ({value})")
        ...
        if not self.widget:
            ...
            for value in self.values:
                ...
                if type_==str:
                    ...
                    value = value
                    value_str = t_to_str(value) if value else ""
                    ui_element = widgets.Text(
                        value=value_str,
                        description=str(i),
                        style={"description_width": "initial"},
